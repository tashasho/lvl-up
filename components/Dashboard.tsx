    ...
    <div className="p-6 pb-24 space-y-8 animate-fade-in">
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-1">{user.goal}</p>
        </div>
        <div className="flex flex-col items-end">
            <div className="w-10 h-10 rounded-full bg-lime-400 flex items-center justify-center font-bold text-black mb-1">
            lvl
            </div>
            <div className="flex items-center gap-1 text-xs font-mono text-lime-400 bg-lime-400/10 px-2 py-1 rounded-full border border-lime-400/20">
      ...
      <div className="flex justify-between items-center gap-4">
        <div className="relative">
            <MacroRing 
                ...
                unit="kcal"
            />
            {kcalHit && <div className="absolute -top-2 -right-2 bg-lime-500 text-black text-[10px] font-bold px-2 py-0.5 rounded-full animate-bounce">HIT!</div>}
        </div>
        <div className="relative">
            <MacroRing 
                ...
                unit="g"
            />
            {proteinHit && <div className="absolute -top-2 -right-2 bg-blue-500 text-black text-[10px] font-bold px-2 py-0.5 rounded-full animate-bounce">HIT!</div>}
        </div>
      </div>
        ...
        {isLoggingWeight && (
            <div className="flex gap-2 mb-6 animate-fade-in">
                <input 
                    ...
                    placeholder="kg"
                    className="flex-1 bg-zinc-800 border border-zinc-700 rounded-xl px-4 text-white focus:outline-none focus:border-lime-400"
                />
                <Button onClick={handleWeightSubmit} className="!py-2">Save</Button>
            </div>
        )}
        ...
        <div className="space-y-4">
             <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                     ...
                     <div>
                         <p className="text-sm text-zinc-200">Apple Health</p>
                         <p className="text-xs text-zinc-500">Syncing steps...</p>
                     </div>
                </div>
                <div className="w-2 h-2 rounded-full bg-lime-500 shadow-[0_0_8px_rgba(163,230,53,0.8)]"></div>
             </div>
        </div>
      </div>
    </div>
